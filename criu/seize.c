...
static enum freezer_state freezer_state = FREEZER_ERROR;

const char *get_real_freezer_state(void)
{
	return freezer_state == THAWED ? thawed : frozen;
}
...
static int freezer_restore_state(void)
{
	...
	if (!opts.freeze_cgroup || freezer_state != FROZEN)
		return 0;
...
static int freeze_processes(void)
{
	...
	freezer_state = state == FREEZING ? FROZEN : state;
...
err:
	if (exit_code == 0 || freezer_state == THAWED)
